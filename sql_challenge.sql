...
SELECT DISTINCT last_name, count(last_name) AS 'Occurrences' 
FROM actor
GROUP BY last_name;
...
SELECT s.staff_id, s.first_name, s.last_name, sum(amount) AS 'Total Amount Rung' 
FROM staff s
...
SELECT title AS 'Film', count(actor_id) AS 'Actor Count' 
FROM film
...
SELECT count(inventory_id) AS '"Hunchback Impossible" Inventory' 
FROM film 
...
SELECT c.first_name, c.last_name, sum(amount) AS 'Total Amt Paid' 
FROM customer c
INNER JOIN payment p
USING (customer_id)
...
SELECT CONCAT(first_name, ' ', last_name) AS 'Customer Name', email AS 'Customer Email' 
FROM customer
...
SELECT title AS 'Film Title', count(rental_id) AS 'Number of Times Rented' 
FROM film
...
SELECT store_id, CONCAT('$', FORMAT(SUM(amount), 2)) AS 'Total AMOUNT (GDP)' 
FROM store
...
SELECT name AS 'Genres', sum(amount) 'Gross Revenue' 
FROM category
